...
@implementation FWLaunchAdCache
...
{
    NSString *path = [NSString stringWithFormat:@"%@/%@",[self fwLaunchAdCachePath],[self keyWithURL:url]];
    if (data)
    {
        BOOL result = [[NSFileManager defaultManager] createFileAtPath:path contents:data attributes:nil];
        if (!result) ADNSLog(@"cache file error for URL: %@", url);
        return result;
    }
    return NO;
}
...
{
    NSString *savePath = [[self fwLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
...
{
    NSString *path =[NSHomeDirectory() stringByAppendingPathComponent:@"Library/FWLaunchAdCache"];
...
{
    if(url==nil) return nil;
    return [[self fwLaunchAdCachePath] stringByAppendingPathComponent:[self keyWithURL:url]];
}

+ (NSString *)videoPathWithURL:(NSURL *)url
{
    if(url==nil) return nil;
    return [[self fwLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
}

+ (NSString *)videoPathWithFileName:(NSString *)videoFileName
{
    if(videoFileName.length==0) return nil;
    return [[self fwLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:[NSURL URLWithString:videoFileName]]];
}
...
+ (void)async_saveImageUrl:(NSString *)url
{
    if(url==nil) return;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[NSUserDefaults standardUserDefaults] setObject:url forKey:FWCacheImageUrlStringKey];
...
{
    return [[NSUserDefaults standardUserDefaults] objectForKey:FWCacheImageUrlStringKey];
}
...
+ (void)async_saveVideoUrl:(NSString *)url
{
    if(url==nil) return;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[NSUserDefaults standardUserDefaults] setObject:url forKey:FWCacheVideoUrlStringKey];
        [[NSUserDefaults standardUserDefaults] synchronize];
    });
}
...
{
    return [[NSUserDefaults standardUserDefaults] objectForKey:FWCacheVideoUrlStringKey];
}
...
+ (void)clearDiskCache
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *path = [self fwLaunchAdCachePath];
        [fileManager removeItemAtPath:path error:nil];
        [self checkDirectory:[self fwLaunchAdCachePath]];
    });
}
...
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *allFilePaths = [self allFilePathWithDirectoryPath:[self fwLaunchAdCachePath]];
        ...
        [allFilePaths enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if(![exceptImagePaths containsObject:obj] && !AdIsVideoTypeWithPath(obj)){
                [[NSFileManager defaultManager] removeItemAtPath:obj error:nil];
            }
        }];
        ADNSLog(@"allFilePath = %@",allFilePaths);
    });
}
...
+ (void)clearDiskCacheExceptVideoUrlArray:(NSArray<NSURL *> *)exceptVideoUrlArray
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [allFilePaths enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if(![exceptVideoPaths containsObject:obj] && AdIsVideoTypeWithPath(obj)){
                [[NSFileManager defaultManager] removeItemAtPath:obj error:nil];
            }
        }];
        ADNSLog(@"allFilePath = %@",allFilePaths);
...
+ (float)diskCacheSize
{
    NSString *directoryPath = [self fwLaunchAdCachePath];
    BOOL isDir = NO;
...
+ (void)createBaseDirectoryAtPath:(NSString *)path
{
    ...
    if (error)
    {
        ADNSLog(@"create cache directory failed, error = %@", error);
    }
    else
    {
        [self addDoNotBackupAttribute:path];
    }
    ADNSLog(@"fwLaunchAdCachePath = %@",path);
}
...
+ (void)addDoNotBackupAttribute:(NSString *)path
{
    ...
    if (error)
    {
        ADNSLog(@"error to set do not backup attribute, error = %@", error);
    }
}
